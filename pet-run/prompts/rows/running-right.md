Create one horizontal animation strip for Codex pet `reef`, state `running-right`.
...
State action: Brisk sailor deck-hurry loop traveling screen-right: Reef faces right and alternates a compact sea-legged stride while carrying one neatly coiled mooring rope hugged securely against his torso. The rope must remain one connected opaque component touching both hands and body in every frame.
