Create one horizontal animation strip for Codex pet `reef`, state `jumping`.
...
State action: Sea-legs deck hop loop: Reef braces in a compact crouch as if the vessel rolls, springs straight upward with knees tucked slightly and tail balancing close to the body, reaches one clean airborne peak, descends, and settles into the same stance. Body motion only; no deck, water, spray, rope, shadows, or effects.
