Create one horizontal animation strip for Codex pet `reef`, state `waiting`.
...
State action: Mooring-order waiting loop: Reef holds a single coiled mooring rope securely between both hands at chest height, glances expectantly toward the viewer, tilts his head, and opens one palm slightly while the rope remains supported and touching his other hand and torso. The expression clearly asks for the next order.
